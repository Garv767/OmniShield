    ...
    <div className="p-8 h-full flex flex-col space-y-6 bg-slate-50 text-slate-900 font-sans overflow-y-auto">
      ...
      <div className="flex-1 flex flex-col max-w-2xl min-h-0 space-y-6">
        <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-3">
          <div className="text-slate-700 uppercase text-[9px] font-bold border-b border-slate-200 pb-2 flex justify-between">
            <span>Required CSV Fields</span>
            <span>CSV Ingestion Standard</span>
          </div>
          <code className="block select-all bg-slate-50 p-2.5 border border-slate-200 text-slate-700 font-mono text-[11px] overflow-x-auto whitespace-nowrap rounded-md">
        ...
        <div className="space-y-4">
          ...
          <button
             ...
             className="w-full py-8 border-2 border-dashed border-slate-200 hover:border-lime-primary/50 bg-white hover:bg-slate-50 text-xs text-slate-650 hover:text-slate-800 transition flex flex-col items-center justify-center space-y-2.5 cursor-pointer rounded-xl shadow-sm"
          >
             {csvUploading ? (
                <Loader className="w-5 h-5 text-lime-primary animate-spin" strokeWidth={1.5} />
             ) : (
                <Upload className="w-5 h-5 text-slate-700" strokeWidth={1.5} />
             )}
             <span className="font-bold text-xs tracking-wide">
                {csvUploading ? 'Processing CSV Records...' : 'Load Complaints CSV File'}
             </span>
             <div className="flex flex-col items-center space-y-0.5 text-[10px] text-slate-650 font-medium">
                <span>Supported format: RFC 4180 compliant CSV</span>
                <span className="text-[9px] text-slate-600">Maximum file size: 50MB. Records will be queued in PostgreSQL.</span>
             </div>
          </button>
